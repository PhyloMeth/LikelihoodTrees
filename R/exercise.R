...
InferMorphologyTree_exercise <- function(in.place=FALSE, input.path="~/Desktop/LikelihoodTrees/inst/extdata", input.file = "binary.phy", output.path="~/Desktop/LikelihoodTrees", output.name = "morpho1", random.seed=12345, model="ASC_BINGAMMA", other='--asc-corr=lewis') {
	if(!in.place) {
		if(is.null(input.path)) {
			fpath <- system.file("extdata", input.file, package="LikelihoodTrees")
...
InferDNATreeWithBootstrappingAndPartitions_exercise <- function (in.place=FALSE, input.path="~/Desktop/LikelihoodTrees/inst/extdata", input.file = "dna.phy", input.partition = "dna12_3.partition.txt", output.path="~/Desktop/LikelihoodTrees", output.name = "dna1", random.seed=12345, boot.seed=12345, model="GTRGAMMA", boot=100) {
	if(!in.place) {
		if(is.null(input.path)) {
			fpath <- system.file("extdata", input.file, package="LikelihoodTrees")
			system(paste("cp ", fpath, " ", output.path,"/", input.file, sep=""))
			fpath <- system.file("extdata", input.partition, package="LikelihoodTrees")
	...
	raxml.call <- paste("raxmlHPC -m ", model, " -p ", random.seed, " -x ", boot.seed, " -q ", input.partition, " -# ", boot, " -s ", input.file, " -n ", output.name, sep="")
	status <- system(raxml.call)
